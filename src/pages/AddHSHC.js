        ...
        files.forEach((file) => {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    // Lấy sheet Bìa để đọc mã kế hoạch và POP
                    const sheetBia = workbook.Sheets['Bìa'] || workbook.Sheets['BIA'] || workbook.Sheets['bia'];
                    
                    // Lấy danh sách mã kế hoạch từ E27-E29
                    const maKeHoachArr = [];
                    if (sheetBia) {
                        const cells = ['E27', 'E28', 'E29'];
                        for (const cell of cells) {
                            const value = sheetBia?.[cell]?.v;
                            if (value) maKeHoachArr.push(value);
                        }
                    }
                    
                    // Lấy danh sách POP từ F27-F29
                    const popArr = [];
                    if (sheetBia) {
                        const cells = ['F27', 'F28', 'F29'];
                        for (const cell of cells) {
                            const value = sheetBia?.[cell]?.v;
                            if (value) popArr.push(value);
                        }
                    }
                            ...
                            for (let i = 0; i < config.soLuongColumns.length && i < maKeHoachArr.length; i++) {
                                ...
                                if (soLuong > 0) {
                                    ...
                                    processedData.push({
                                        fileName: file.name,
                                        fileType: fileType,
                                        maKeHoach: maKeHoachArr[i] || maKeHoachArr[0] || '',
                                        pop: popArr[i] || popArr[0] || '',
                                        hangMuc: hangMuc,
                                        donGia: donGia,
                                        soLuong: soLuong,
                                        thanhTien: thanhTien
    ...
    const sendDataToAppSheet = async () => {
        ...
        try {
            ...
            const rows = hsHcData.map(item => ({
                "Tên file": item.fileName, // Thêm trường Tên file
